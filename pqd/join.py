...
class PairHeap:
    ...
    def _siftdown(self, i):
        ...
        if self.pairs[max_child] > self.pairs[i]:
            self._parent_swap(max_child)
            self._siftdown(max_child)
        ...
        for pfile in self.layout:
            ...
            for obj_num in range(split_factor):
                ...
                for i in range(-(-size // chunk_size)):
                    ...
                    if verbose:
                        print(f"Making a chunk with {eff_dframes[file_index][obj].shape[0]} rows out of {size} from object {obj} and assigning it to {file_index}")
                    df_index += chunk_size
